...
import { loadTasks, unloadTasks } from "../actions/tasks";
  ...
  render() {
    ...
    if (error) {
      ...
      const tasksNode = (tasks || []).map(task => (
        <ListItem key={`task_${task.id}`} justify="between">
          <Label>
            <Anchor path={`/tasks/${task.id}`} label={task.name} />
          </Label>
